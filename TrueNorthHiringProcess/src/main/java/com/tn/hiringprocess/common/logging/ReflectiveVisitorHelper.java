    ...
    private static class ClassVisitMethods {
        ...
        private Method findNullVisitorMethod() {
            for (Class clazz = this .visitorClass; clazz != null; clazz = clazz
                    .getSuperclass()) {
                try {
                    return clazz.getDeclaredMethod(VISIT_NULL,
                            (Class[]) null);
                ...
                try {
                    if (logger.isTraceEnabled()) {
                        logger.trace("Looking for method "
                                + VISIT_METHOD + "(" + argumentType
                                + ")");
                    }
                    return findVisitMethod(this .visitorClass,
                            argumentType);
                    ...
                    if (!argumentType.isInterface()
                        ...
                        classQueue.addFirst(argumentType
                                .getSuperclass());
                    }
        ...
        private Method findVisitMethod(Class visitorClass,
            ...
            try {
                ...
                if (visitorClass.getSuperclass() != Object.class) {
                    return findVisitMethod(
                            visitorClass.getSuperclass(), argumentType);
                } else {
                    throw ex;
                }
            }
